...
void logger(int type, char *s1, char *s2, int socket_fd)
{
    ...
    {
    case ERROR:
        (void)sprintf(logbuffer, "Time:%sRROR: %s:%s Errno=%d exiting pid=%d", ctime(&now_time), s1, s2, errno, getpid());
        break;
    case FORBIDDEN:
        (void)write(socket_fd, "HTTP/1.1 403 Forbidden\nContent-Length: 185\nConnection: close\nContent-Type: text/html\n\n<html><head>\n<title>403 Forbidden</title>\n</head><body>\n<h1>Forbidden</h1>\n The requested URL, file type or operation is not allowed on this simple static file webserver.\n</body></html>\n", 271);
        (void)sprintf(logbuffer, "Time:%sFORBIDDEN: %s:%s", ctime(&now_time), s1, s2);
        break;
    case NOTFOUND:
        (void)write(socket_fd, "HTTP/1.1 404 Not Found\nContent-Length: 136\nConnection: close\nContent-Type: text/html\n\n<html><head>\n<title>404 Not Found</title>\n</head><body>\n<h1>Not Found</h1>\nThe requested URL was not found on this server.\n</body></html>\n", 224);
        (void)sprintf(logbuffer, "Time:%sNOT FOUND: %s:%s", ctime(&now_time), s1, s2);
        break;
    case LOG:
        (void)sprintf(logbuffer, "Time:%s INFO: %s:%s:%d", ctime(&now_time), s1, s2, socket_fd);
        break;
    }
